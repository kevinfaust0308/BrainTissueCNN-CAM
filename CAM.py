...
# 3. overlay_cam_large_image
# 4. overlay_multi_stacked_cam_large_image
##############################################################
...
def predict_label_with_cam(model, image, conv_name, class_idx=-1, overlay=False, overlay_alpha=0.5,
                           cmap=None):
    '''   
...
def overlay_cam_large_image(model, cnn_trained_image_size, image, conv_name, class_idx, overlay_alpha=0.5,
    ...
    for i in list(range(0,(width-cnn_trained_image_size)+1,cnn_trained_image_size)):
        ...
        for j in list(range(0,(height-cnn_trained_image_size)+1,cnn_trained_image_size)):
            ...
            overlay_cam, _ = predict_label_with_cam(model, sub_img, conv_name, class_idx, overlay=True, overlay_alpha=overlay_alpha, cmap=cmap)
...
def overlay_prediction_on_image(image, pred): 
    '''
    Takes a CAM overlayed image and writes predictions over it
    '''
    ...
    for obj in pred:
        ...
        p = '{:.2f}%'.format(pred[obj][0]*100)
        # our new list will have object name, acc
        my_list.append([obj] + [p])
    
    
    # text color. bright green
    text_color = (243,243,21)
    # starting top offset
    top_offset = 30
...
def overlay_multi_stacked_cam_large_image(model, cnn_trained_image_size, classes, image, conv_name, overlay_alpha=0.5):
    ''' 
    ...
    for i in list(range(0,(width-cnn_trained_image_size)+1,cnn_trained_image_size)):
        ...
        for j in list(range(0,(height-cnn_trained_image_size)+1,cnn_trained_image_size)):
            ...
            # get cam of desired class
            overlay_cam, pred = get_multi_stacked_cam(model, sub_img, classes, conv_name, overlay_alpha=overlay_alpha, threshold=0.3,
                          show_top_x_classes=IMAGES_TO_SHOW, pretty_top_predictions=True)
            
            # show on subimage the top 3 predictions 
            overlay_prediction_on_image(overlay_cam, pred)
